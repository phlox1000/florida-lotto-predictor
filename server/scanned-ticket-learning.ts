...
export async function evaluateConfirmedScannedTicketsForDraw(
  ...
  return withMySqlNamedLock(
    ...
    },
    {
      onLockMiss: () => {
        console.warn(
          `[ScannedTicketLearning] skipped evaluateConfirmedScannedTicketsForDraw due to lock contention drawId=${drawId} gameType=${gameType}`
        );
      },
      fallbackResult: { evaluatedCount: 0, newOutcomes: 0 },
    }
  );
}
